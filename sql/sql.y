...
column_list:  NAME                    { emit("COLUMN %s", $1); free($1); $$ = 1; }
            | STRING                  { emit("COLUMN %s", $1); free($1); $$ = 1; }
            | column_list ',' NAME    { emit("COLUMN %s", $3); free($3); $$ = $1 + 1; }
            | column_list ',' STRING  { emit("COLUMN %s", $3); free($3); $$ = $1 + 1; }
...
create_table_stmt: CREATE opt_temporary TABLE opt_if_not_exists NAME '(' create_col_list ')'
            ...
            | CREATE opt_temporary TABLE opt_if_not_exists NAME '(' create_col_list ')' create_select_statement
            ...
            | CREATE opt_temporary TABLE opt_if_not_exists NAME '.' NAME '(' create_col_list ')' create_select_statement
